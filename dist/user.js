...
$(document).on('click', '.set_oper .btn_cancel', function () {
    ...
    var ids = $.map($('.set_left .left_list .left_list_item'), function (item) {
        return $(item).data('id');
    });
    alert(ids);
    ...
    if ($(this).is('.checked')) {
        var $li = $('<li class="left_list_item" data-id="' + $(this).data('id') + '">            <i class= "icon ' + $(this).data("icon") + '" ></i>            <span class="menu_name">' + $(this).find('.menu_name_en').text() + '</span>        </li>');
...
//# sourceMappingURL=.maps/user.js.map
